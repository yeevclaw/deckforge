...
- **`templates/`** — 56 SVG starting templates + `_base.svg` (shared filters/gradients + the Lucide icon set referenced via `<use>`) = **57 files total**, all `viewBox="0 0 1280 720"`. Includes the `IT_prism` family (`prism_*`, v0.16.0 — the default style: 15 files — reeded-glass cover + 14 starters, mirroring the fresh set) and the `corporate_fresh` family (`fresh_*`), each with four `*_flow*` glass-flow variants and the per-page `card_variant` families `*_3col*` (4), `*_mini_grid*` (3), `*_2col*` (2) for `three_col` / `mini_grid` / `two_col_50_50`; and the ten `chart_*` files — the basic three (bar/line/donut), the consulting five (v0.14.0 think-cell vocabulary: hbar/stacked_bar/waterfall/combo/mekko), plus two specialized (v0.15.0: `chart_radar` multi-axis assessment + `chart_gantt` project schedule); annotation recipes in `references/chart_anatomy.md`.
