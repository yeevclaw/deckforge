...
- **`templates/`** — 39 SVG starting templates + `_base.svg` (shared filters/gradients + the Lucide icon set referenced via `<use>`) = **40 files total**, all `viewBox="0 0 1280 720"`. Includes the `corporate_fresh` family (`fresh_*`), whose four `fresh_flow*` files are the glass-flow variants and whose `fresh_3col*` (4), `fresh_mini_grid*` (3), and `fresh_2col*` (2) files are the per-page `card_variant` families for `three_col` / `mini_grid` / `two_col_50_50`; and the eight `chart_*` files — the basic three (bar/line/donut) plus the consulting five (v0.14.0 think-cell vocabulary: hbar/stacked_bar/waterfall/combo/mekko, annotation recipes in `references/chart_anatomy.md`).
