...
**The harness.** In the LangChain "loop engineering" sense, the *harness* is everything wrapping the model — for DeckForge that's `SKILL.md` + `prompts/*` + `references/*` + `templates/*` + `scripts/svg_to_pptx.py`. Its gradeable quality bar is `references/rubric.md`, scored independently in the Phase 5 verification loop (`prompts/06_visual_grader.md`); its hill-climbing improvement procedure is `references/improvement_loop.md`. The per-file breakdown:
