...
- **`references/`** — knowledge bases the prompts cite: `bento_grid.md` (8 base layouts, the default), `diagrams.md` (9 diagram primitives, used only when bento loses structural information), `chart_anatomy.md`, `design_system.md` (palettes/typography; `dark_apple` family is the default), `socratic_loop.md`, `pyramid_principle.md`, `editable_mode.md`.
- **`templates/`** — 20 SVG starting points, all `viewBox="0 0 1280 720"`. `_base.svg` holds shared filters/gradients and the Lucide icon set referenced via `<use>`.
