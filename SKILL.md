...
- Speaker notes (1–2 sentences per page; on reading-mode pages the reader-facing support goes on the page itself — `reading_notes` / body lines — not into speaker notes)
...
"layout":
  ...
  // …and two specialized (multi-axis assessment + project schedule):
...
1. **Grade** — spawn a fresh grader sub-agent with [prompts/07_content_grader.md](prompts/07_content_grader.md). Give it `planning.json`, `brief.md`, and [references/rubric.md](references/rubric.md) (it scores the Phase 3 content criteria **P3-11 / P3-12 / P3-13** — AI filler, pyramid alignment, title-only read — plus **P3-19**, reading-mode routing, on pages whose effective delivery mode is reading; it reads the mode from `planning.json`). It grades whether the plan faithfully carries the **already-approved** thesis into load-bearing cards; it does **not** reopen the Socratic dialogue or second-guess the thesis itself (settled and user-approved in Phase 1). It returns strict JSON: `{ "plan_pass": bool, "title_read": {…}, "pages": [ { "n", "pass", "failures": [ { "rubric_id", "where", "fix" } ] } ] }`. The grader did **not** write the plan — that independence is the point. Save the returned JSON verbatim to `_qa/grade_p3.json` in the working dir, so the verdict survives the session (a re-grade overwrites it — the last verdict is the record).
...
- SVG templates to start from: [templates/](templates/) — 59 files total:
  ...
  - **Chart layouts**: `chart_bar.svg`, `chart_line.svg`, `chart_donut.svg`, plus the consulting five — `chart_hbar.svg` (ranking), `chart_stacked_bar.svg` (mix shift: totals + segment connectors + emphasis series), `chart_waterfall.svg` (A→B bridge + diff bracket), `chart_combo.svg` (bars + ink rate line + CAGR arrow), `chart_mekko.svg` (2-D market map), and two specialized — `chart_radar.svg` (multi-axis assessment profile), `chart_gantt.svg` (project schedule / roadmap)
  - **IT_prism starters** (the default family — embed its craft recipes; use these when `palette_hint` is `IT_prism`): `prism_cover.svg` (reeded-glass cover / derive end page), four `three_col` `card_variant` templates: `prism_3col.svg` (`icon_column`), `prism_3col_steps.svg` (`numbered_steps`), `prism_3col_axis.svg` (`axis_labeled`), `prism_3col_lead.svg` (`lead_plus_pair`); three `mini_grid` KPI templates: `prism_mini_grid.svg` (`even_grid`), `prism_mini_grid_ribbon.svg` (`ribbon_row`), `prism_mini_grid_spotlight.svg` (`spotlight`); two `two_col_50_50` templates: `prism_2col.svg` (`balanced`), `prism_2col_beforeafter.svg` (`before_after`); `prism_compare.svg` (compare_table), four flow templates — one per `design_brief.flow_variant`: `prism_flow_terrace.svg` (`terrace_ascent`), `prism_flow_river.svg` (`river_ribbon`), `prism_flow_cascade.svg` (`cascade_fall`), `prism_flow.svg` (`dome_arcade`), and `prism_doc.svg` (the reading-mode slidedoc prose starter — see [references/slidedoc.md](references/slidedoc.md))
  - **corporate_fresh starters** (embed that family's craft recipes — use these, not restyled dark templates, when `palette_hint` is `corporate_fresh`): `fresh_cover.svg` (cover / derive end page), four `three_col` `card_variant` templates — one per page's `card_variant`: `fresh_3col.svg` (`icon_column`), `fresh_3col_steps.svg` (`numbered_steps`), `fresh_3col_axis.svg` (`axis_labeled`), `fresh_3col_lead.svg` (`lead_plus_pair`); three `mini_grid` KPI templates: `fresh_mini_grid.svg` (`even_grid`), `fresh_mini_grid_ribbon.svg` (`ribbon_row`), `fresh_mini_grid_spotlight.svg` (`spotlight`); two `two_col_50_50` templates: `fresh_2col.svg` (`balanced`), `fresh_2col_beforeafter.svg` (`before_after`); `fresh_compare.svg` (compare_table), four flow templates — one per `design_brief.flow_variant`: `fresh_flow_terrace.svg` (`terrace_ascent`), `fresh_flow_river.svg` (`river_ribbon`), `fresh_flow_cascade.svg` (`cascade_fall`), `fresh_flow.svg` (`dome_arcade`), and `fresh_doc.svg` (the reading-mode slidedoc prose starter — see [references/slidedoc.md](references/slidedoc.md))
...
Key rules:
- **Default style — `IT_prism`**: when the user doesn't specify a style, use the IT-prism cool light look (lavender-tinted `#EFF0F3` canvas with a diagonal lavender→mint wash, slate `#344252` assertion titles, white panel cards, one green accent `#58D494` in shapes only, slate line icons, `corner_bloom` + `progress_ticks` page signature, reeded-glass cover/end whose hues never reach content pages — flow pages varied per deck via `design_brief.flow_variant`). `palette_hint: "IT_prism"`, `highlight_color: "#58D494"`, `motif_hint: "prism_panel_cards"`. Full spec + composition vocabulary: [references/design_system.md](references/design_system.md) → "IT prism family". `corporate_fresh` (the warm consulting look) stays available on request.
...
- **Pick the palette AND the single highlight color ONCE** at the start of phase 4 (for `dark_apple*`, auto-detect from brand if possible: Xiaomi `#FF6900`, Tesla `#E31937`, etc.). Reuse on every page. **Never use a second accent color.** (The light families replace the free-choice highlight with fixed role palettes — `IT_prism`: shape-only accent green + slate structure; `corporate_fresh`: structure green / icon blue / emphasis orange; using a color outside its role is the same violation.)
- **Pick one visual motif** (default `prism_panel_cards`; `fresh_pill_cards` for `corporate_fresh`, `apple_dark_cards` for `dark_apple*` decks) and repeat it everywhere.
...
- **One card, one core point.** If a card body has multiple sentences or a heading + 3 bullets, split it into a mini_grid. **Reading-mode pages** (`delivery_mode: "reading"` — deck-level or per-page override): the unit becomes one **argument** — a claim plus 2–4 sentences that develop it — but never a second claim; and Tier-2 supporting content moves from `speaker_notes` onto the page itself (body lines / `sub_cards` / `reading_notes`), because a reader never opens `.notes.md`. See `prompts/04_planning_draft.md` → "Delivery mode".
- **Number-first cards beat text-first cards.** Whenever a key number captures the message, render it large. **`dark_apple`**: 80–120px in the highlight color. **`IT_prism` (default)**: a hinge number is slate `#344252` (or white on a slate anchor block) — **never** green (≈1.9:1 on white; green is shape-only). **`corporate_fresh`**: ink `#383838` or teal `#1B8A82` — never the orange highlight. See `prompts/05_designer_svg.md` → the per-family `stat_hero` overrides.
- **Bilingual title pattern.** **`dark_apple`**: page titles render as Chinese (big, white) + small English subtitle (gray, decorative). **Light families (`IT_prism` default / `corporate_fresh`)**: the title is a full-sentence assertion at 30–36px weight 700 (slate `#344252` on prism, charcoal `#383838` on fresh — not a white mega-title), with the small English subtitle still gray.
...
2. **Grade** — spawn a fresh grader sub-agent with [prompts/06_visual_grader.md](prompts/06_visual_grader.md). Give it the `_renders/page_NN.png` images, `planning.json`, and [references/rubric.md](references/rubric.md) (it scores the "Phase 5 — VISUAL" section, ids P5-01..P5-08, P5-10, and P5-11 on chart pages; it reads each page's effective delivery mode from `planning.json` — P5-08's bar is mode-aware, so a correct reading-mode slidedoc page is never failed for intended density). It returns strict JSON:
   ...
   ```
   The grader did **not** design the slides — that independence is the point; a same-context self-check rationalizes its own output. Save the returned JSON verbatim to `_qa/grade_p5.json` in the working dir, so the verdict survives the session (a re-grade overwrites it — the last verdict is the record).
...
4. **Phase 4**: Read `planning.json`. Render all pages with the deck's style — `IT_prism` by default; `corporate_fresh` / `dark_apple` (or a brand palette) only if the user picked it in Phase 1.
...
│   ├── chart_radar.svg               ← multi-axis assessment profile (3–8 axes)
│   ├── chart_gantt.svg               ← project schedule / roadmap (duration bars)
│   ├── prism_cover.svg / prism_3col*.svg / prism_mini_grid*.svg / prism_2col*.svg / prism_flow*.svg / prism_compare.svg
│   │                                 ← IT_prism starters (the default family; craft recipes embedded)
