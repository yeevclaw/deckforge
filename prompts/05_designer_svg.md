...
8. **Bento Grid spacing**: main cards keep a ≥48px outer margin from the canvas edge and ≥20px gaps between each other; mini-cards inside a main card have ≥24px gaps and ≥40px main-card inner padding.
...
**Marking rules & numeric baselines** (the converter warns on the first three; the rest are design law):
...
- **cover**: the `reeded_glass` recipe (design_system.md → IT prism family) — near-white base, 2–3 gaussian-blurred color blobs (lavender `#C9C6FF` circle / mint `#8AEBD2` block / cyan `#B8EEFF` haze) behind a vertical fluted white-line panel on the right ~55%; clear left column carries a slate `#344252` chip (white 14–16px label), the CN title 56–64px weight 700 in **slate** (never white-on-gradient, never green), a `#4C5A6B` date/department line (on-canvas text — `#6B7686` is a white-card token and fails AA on the wash). These blob hues live on cover + end pages ONLY.
