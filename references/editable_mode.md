...
- **Gradient fills used as backgrounds / washes / glows** (`fill="url(#…)"`) and any **translucent** decorative shape (`opacity`/`fill-opacity` < 1, e.g. glass arches, Venn overlaps).
...
- **Animated (flow-anim) slides** are embedded as a looping GIF and are **not** Convert-to-Shape editable (the GIF is the whole slide).
